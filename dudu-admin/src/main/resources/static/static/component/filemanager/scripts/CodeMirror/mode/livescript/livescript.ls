...
CodeMirror.defineMode 'livescript', (conf) ->
  tokenBase = (stream, status) ->
    #indent =
    if next_rule = status.next or \start
  ...
  external = {
    ...
    token: (stream, status) ->
      style = tokenBase stream, status #tokenLexer stream, status
      status.lastToken = {
        style: style
      ...
      style.replace /\./g, ' '
    indent: (status, textAfter) ->
      # XXX this won't work with backcalls
      indentation = status.lastToken.indent
      if status.lastToken.content.match indenter then indentation += 2
      return indentation
  }
  external
