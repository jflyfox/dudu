...
var menuItem = Vue.extend({
	...
	template:[
	          ...
	          '<a v-if="item.type === 1" href="javascript:;" :data-url="dudu.ctx + item.url"><i v-if="item.icon != null" :class="item.icon" :data-icon="item.icon"></i> <span>{{item.name}}</span></a>',
	          '</dd>',
	          '</dl>',
	          '<a v-if="item.type === 1" href="javascript:;" :data-url="dudu.ctx + item.url"><i v-if="item.icon != null" :class="item.icon" :data-icon="item.icon"></i> <span>{{item.name}}</span></a>',
	          '</li>'
...
var vm = new Vue({
	...
	methods: {
		getMenuList: function (event) {
			$.getJSON(dudu.ctx + "/system/menu/user?_"+$.now(), function(r){
				vm.menuList = r.data;
			});
		},
		getUser: function(){
			$.getJSON(dudu.ctx + "/system/user/info?_"+$.now(), function(r){
				vm.user = r.data;
			});
		},
